...
describe("StackingManager", function (accounts) {
    ...
    beforeEach(async function () {
        ...
        // stackingManager deployment
        let StackingManager = await hre.ethers.getContractFactory("StackingManager");
        stackingManager = await StackingManager.deploy(tokenPHARM.address, priceProvider.address);

        // Pre-minting of PHARM tokens
        await tokenPHARM.mint(stackingManager.address, PHARM_STAKING_REWARDS_SUPPLY);
        ...
        // setting PHARM approval for stackingManager
        await tokenPHARM.connect(addr1).approve(stackingManager.address, PHARM_PREFUND_SUPPLY);
        await tokenPHARM.connect(addr2).approve(stackingManager.address, PHARM_PREFUND_SUPPLY);
        await tokenPHARM.connect(addr3).approve(stackingManager.address, PHARM_PREFUND_SUPPLY);
        await tokenPHARM.connect(addr4).approve(stackingManager.address, PHARM_INSUFFICENT_ALLOWED_AMOUNT);
    });
        ...
        context('Staking', () => {
            ...
            describe("stakePHARM()", () => {
                ...
                it("should allow a user to stake PHARM tokens", async function () {
                    await stackingManager.connect(addr1).stakePHARM(PHARM_SUFFICENT_AMOUNT);
                    let user = await stackingManager.connect(addr1).getUser(addr1.address);
                ...
                it("should forbid a user to stake an unsuficient amount of PHARM tokens", async function () {
                    await expect(stackingManager.connect(addr1).stakePHARM(PHARM_INSUFFICENT_AMOUNT))
                ...
                it("should forbid a user to stake more PHARM tokens than he has", async function () {
                    await expect(stackingManager.connect(addr1).stakePHARM(PHARM_SUFFICENT_AMOUNT + "1"))
                ...
                it("should forbid a user to stake more PHARM tokens than he allowed the contract to transfer", async function () {
                    await expect(stackingManager.connect(addr4).stakePHARM(PHARM_PREFUND_SUPPLY))
                        .to.be.revertedWith("PHARM: insufficient allowance.");
                });

                it("should emit a StakePHARM event", async function () {
                    await expect(stackingManager.connect(addr1).stakePHARM(PHARM_SUFFICENT_AMOUNT))
                        .to.emit(stackingManager, "StakePHARM")
            ...
            describe("stakeETH()", () => {
                it("should allow a user to stake ETH tokens", async function () {
                    await stackingManager.connect(addr1).stakeETH({value: ETH_SUFFICENT_AMOUNT});
                    let user = await stackingManager.connect(addr1).getUser(addr1.address);
                ...
                it("should forbid a user to stake an unsuficient amount of ETH tokens", async function () {
                    await expect(stackingManager.connect(addr1).stakeETH({value: ETH_INSUFFICENT_AMOUNT}))
                        .to.be.revertedWith("ETH: minimum stake amount is 0.1 ETH.");
                });

                it("should emit a StakeETH event", async function () {
                    await expect(stackingManager.connect(addr1).stakeETH({value: ETH_SUFFICENT_AMOUNT}))
                        .to.emit(stackingManager, "StakeETH")
        ...
        context('Unstaking', () => {
            ...
            describe("unstakePHARM()", () => {
                it("should allow a user to unstake PHARM tokens", async function () {
                    await stackingManager.connect(addr1).stakePHARM(PHARM_SUFFICENT_AMOUNT);
                    await stackingManager.connect(addr1).unstakePHARM(PHARM_SUFFICENT_AMOUNT);
                    let user = await stackingManager.connect(addr1).getUser(addr1.address);
                ...
                it("should forbid a user to unstake more PHARM tokens than he has staked", async function () {
                    await stackingManager.connect(addr1).stakePHARM(PHARM_SUFFICENT_AMOUNT);
                    await expect(stackingManager.connect(addr1).unstakePHARM(PHARM_SUFFICENT_AMOUNT + "1"))
                        .to.be.revertedWith("PHARM: insufficient staked amount.");
                });
                
                it("should emit a UnstakePHARM event", async function () {
                    await stackingManager.connect(addr1).stakePHARM(PHARM_SUFFICENT_AMOUNT);
                    await expect(stackingManager.connect(addr1).unstakePHARM(PHARM_SUFFICENT_AMOUNT))
                        .to.emit(stackingManager, "UnstakePHARM")
                        .withArgs(addr1.address, PHARM_SUFFICENT_AMOUNT);
                });
            });
            ...
            describe("unstakeETH()", () => {
                it("should allow a user to unstake ETH tokens", async function () {
                    ...
                    await stackingManager.connect(addr1).unstakeETH(ETH_SUFFICENT_AMOUNT);
                ...
                it("should forbid a user to unstake more ETH tokens than he has staked", async function () {
                    await stackingManager.connect(addr1).stakeETH({value: ETH_SUFFICENT_AMOUNT});
                    await expect(stackingManager.connect(addr1).unstakeETH(ETH_SUFFICENT_AMOUNT + "1"))
                        .to.be.revertedWith("ETH: insufficient staked amount.");
                });
                it("should emit a UnstakeETH event", async function () {
                    await stackingManager.connect(addr1).stakeETH({value: ETH_SUFFICENT_AMOUNT});
                    await expect(stackingManager.connect(addr1).unstakeETH(ETH_SUFFICENT_AMOUNT))
                        .to.emit(stackingManager, "UnstakeETH")
                        .withArgs(addr1.address, ETH_SUFFICENT_AMOUNT);
                });
            });
        ...
        context('Rewards', () => {
            ...
            describe("claimRewards()", () => {
                it("should allow a user to claim his rewards", async function () {
                    ...
                    await stackingManager.connect(addr1).unstakePHARM(PHARM_SUFFICENT_AMOUNT);
                    await stackingManager.connect(addr1).claimRewards();
                ...
                it("should emit a RewardsClaimed event", async function () {
                    ...
                    // time forwarding
                    await time.increaseTo(await time.latest() + 12 * month);
                    await expect(stackingManager.connect(addr1).claimRewards())
                        .to.emit(stackingManager, "RewardsClaimed")
                });
            });
    ...
    context('Reward Fuzzing', () => {
        ...
        it("should aproximatively rewarding 80 PHARM tokens for 1000 PHARM staked on one year", async function () {
            await stackingManager.connect(addr1).stakePHARM(ethers.utils.parseEther("1000"));
        ...
        it("should aproximatively rewarding 40 PHARM tokens for 1000 PHARM staked on six months", async function () {
            ...
            await stackingManager.connect(addr1).unstakePHARM(ethers.utils.parseEther("1000"));
            let user = await stackingManager.connect(addr1).getUser(addr1.address);
        ...
        it("should aproximatively rewarding 40 PHARM tokens for 500 PHARM staked on one year", async function () {
            await stackingManager.connect(addr1).stakePHARM(ethers.utils.parseEther("500"));
            await time.increaseTo(await time.latest() + 1 * year);
            await stackingManager.connect(addr1).unstakePHARM(ethers.utils.parseEther("500"));
            let user = await stackingManager.connect(addr1).getUser(addr1.address);
        ...
        it("should aproximatively rewarding 8 PHARM tokens for 100 PHARM staked on one year", async function () {
            await stackingManager.connect(addr1).stakePHARM(ethers.utils.parseEther("100"));
            await time.increaseTo(await time.latest() + 1 * year);
            await stackingManager.connect(addr1).unstakePHARM(ethers.utils.parseEther("100"));
            let user = await stackingManager.connect(addr1).getUser(addr1.address);
        ...
        it("should aproximatively rewarding 3200 PHARM tokens for 10 ETH staked on one year", async function () {
            await stackingManager.connect(addr1).stakeETH({value: ethers.utils.parseEther("10")});
            await time.increaseTo(await time.latest() + 1 * year);
            await stackingManager.connect(addr1).unstakeETH(ethers.utils.parseEther("10"));
            let user = await stackingManager.connect(addr1).getUser(addr1.address);
        ...
        it("should aproximatively rewarding 320 PHARM tokens for 1 ETH staked on one year", async function () {
            await stackingManager.connect(addr1).stakeETH({value: ethers.utils.parseEther("1")});
            await time.increaseTo(await time.latest() + 1 * year);
            await stackingManager.connect(addr1).unstakeETH(ethers.utils.parseEther("1"));
            let user = await stackingManager.connect(addr1).getUser(addr1.address);
        ...
        it("should aproximatively rewarding 32 PHARM tokens for 0.1 ETH staked on one year", async function () {
            await stackingManager.connect(addr1).stakeETH({value: ethers.utils.parseEther("0.1")});
            await time.increaseTo(await time.latest() + 1 * year);
            await stackingManager.connect(addr1).unstakeETH(ethers.utils.parseEther("0.1"));
            let user = await stackingManager.connect(addr1).getUser(addr1.address);
